...
The incremental builder will do an initial full build and will display a message that includes the phrase "Finished compilation" once the initial build is complete. The initial build may take some time, but subsequent builds will be faster due to incremental compilation.
...
If you encounter issues during the build or run process, try the following:

- Make sure you have installed all the prerequisites listed above
- Run `git clean -xfd` in the `vscode` directory to remove any build artifacts
- Delete the `node_modules` directory and run `yarn` again to reinstall dependencies
- Check the [FAQ](https://github.com/microsoft/vscode/wiki/FAQ) for common issues and solutions
