...
- For [Remote - Containers](https://aka.ms/vscode-remote/download/containers), use the **Remote-Containers: Open Folder in Container...** command after opening the command palette using <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd> (<kbd>CMD</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd> on macOS), which creates a Docker volume for better disk I/O on macOS and Windows.
