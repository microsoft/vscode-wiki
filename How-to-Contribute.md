...
Open source part of VSCode doesn't include the extension gallery URLs. The recommended way to work with extensions is to download the extension directly and side load using the command line or using __Install from VSIX__ command in the Extensions View command drop-down ([see more](https://code.visualstudio.com/docs/editor/extension-gallery#_command-line-extension-management) on command line extension management).
