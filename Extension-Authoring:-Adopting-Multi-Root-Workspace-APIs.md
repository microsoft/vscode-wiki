...
- if the server is using configuration settings the author also has to make the decision which scope applies to the settings (e.g. whether a setting has a 'window' or 'resource' scope). For a language server 'resource' scope is typically preferred, since it enables a user to configure that language settings on a per folder basis.
...
Settings that support relative paths relative to a workspace need special attention. In the single folder case the language server process is started in the root folder of the workspace and a relative path can be resolved easily since the servers home directory corresponds to the workspace. In the multi-root folder setup this is no longer the case. An approach that has worked well for us is to resolve the relative path of a setting on the client so that the server only sees absolute paths. An example for this can be found in [vscode-tslint](https://github.com/Microsoft/vscode-tslint/blob/master/tslint/extension.ts#L200).
